...
function Nav({ page = "home" }) {
  ...
  const [active, setActive] = useState("talks");
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      scrollFrame.current = requestAnimationFrame(() => {
        ...
        const sections = ["talks", "posts", "join"];
        let cur = "talks";
  ...
  // A section link scrolls while we are on the home page and walks back to it
  // when we are not. The people page is a page, so it is always a plain link.
  const sectionHref = (hash) => isHome ? hash : "index.html" + hash;
  const sectionClick = (id) => isHome ? go(id) : () => setMobileOpen(false);
  const here = (id) => isHome && active === id ? "active" : "";
  // The drawer is mounted on every page, so About opens in place; the href is
  // there for anyone arriving without scripts.
  const aboutClick = (e) => {
    e.preventDefault();
    setMobileOpen(false);
    openAbout();
    ...
    <nav className={"nav" + (scrolled ? " scrolled" : "") + (mobileOpen ? " menu-open" : "")} aria-label="Primary navigation">
        <div className="container nav-inner">
          <a className="brand" href={isHome ? "#top" : "index.html"}
            onClick={isHome ? (e) => {e.preventDefault();setMobileOpen(false);window.scrollTo({ top: 0, behavior: "smooth" });} : undefined}
            aria-label="Statistics Study Group home">
          ...
          <div className="nav-links">
            <a href={sectionHref("#about")} onClick={aboutClick}>About</a>
            <a href={sectionHref("#talks")} className={here("talks")} onClick={sectionClick("talks")}>Research Atlas</a>
            <a href={sectionHref("#posts")} className={here("posts")} onClick={sectionClick("posts")}>Posts</a>
            <a href="people.html" className={page === "people" ? "active" : ""}
              aria-current={page === "people" ? "page" : undefined}>Maintainers &amp; Contributors</a>
            <button className="nav-cta" onClick={isHome ? go("join") : () => {window.location.href = "index.html#join";}}>Join Us</button>
          </div>
        ...
        {mobileOpen && (
          <div className="mobile-menu" id="mobile-navigation">
            <div className="container mobile-menu-inner">
              ...
              <a href="people.html" className={page === "people" ? "active" : ""}
                aria-current={page === "people" ? "page" : undefined}>Maintainers &amp; Contributors</a>
              <button type="button" className="mobile-menu-cta" onClick={isHome ? go("join") : () => {window.location.href = "index.html#join";}}>Join the group</button>
            </div>
          </div>
        )}
      </nav>);
            ...
            <div className="hero-grid">
              <div className="hero-text">
                ...
                <div className="hero-actions" aria-label="Explore the study group">
                  ...
                  <a className="hero-action secondary" href="people.html">Meet the group</a>
                </div>
              </div>
              <HeroCarousel />
            </div>
...
const FIELDS = ["MSBE", "FinTech", "Accounting & Finance", "MSBE & Finance", "Finance",
                "Economics & Finance", "Marketing", "School of Mathematics", NO_FIELD];
...
deepsky: {
  ...
  hue: { "MSBE": "#A9DDAE", "FinTech": "#9FC4F5", "Accounting & Finance": "#F5A9A2", "MSBE & Finance": "#D9A6E8", "Finance": "#F7CE90", "Economics & Finance": "#8FDAD6",
         "Marketing": "#F0A8D8", "School of Mathematics": "#B9B0F5", "Field TBC": "#9AA3AD" },
  warm: [255, 184, 99],
  bg(ctx, w, h, s, e, t) {
    ...
    <section id="maintainers" className="section section-people">
      <div className="container">
        <div className="section-header reveal">
          <div className="num">Maintainers & Contributors</div>
...
// -------- THE WALL (anonymous) --------
// Two boards, no accounts: what people would like to learn, and notes and
// chatter. Messages are stored as comments on two issues in the site's own
// repository (api/wall.js); maintainers moderate there by deleting a comment.
function Wall() {
  const BOARDS = [
  { id: "wish", label: "What I'd like to learn", hint: "A method, a paper, a tool, a question - anything you wish the group covered next." },
  { id: "chat", label: "Notes & chatter", hint: "A thought after a session, a reaction, a thank-you. Anything worth saying out loud." }];
  const MAX = 500;
  const [board, setBoard] = useState("wish");
  const [wall, setWall] = useState({ status: "loading", boards: { wish: [], chat: [] }, passphrase: false });
  ...
  useEffect(() => {
    let alive = true;
    fetch("api/wall").then((r) => r.ok ? r.json() : Promise.reject(new Error("HTTP " + r.status))).then((d) => {
      if (!alive) return;
      setWall({ status: d.configured ? "ready" : "offline", boards: d.boards || { wish: [], chat: [] }, passphrase: Boolean(d.passphraseRequired) });
    }).catch(() => { if (alive) setWall((w) => ({ ...w, status: "offline" })); });
    return () => { alive = false; };
  }, []);
  ...
  const post = async (e) => {
    ...
    if (body.length < 2) { setNote({ kind: "bad", text: "Write something first." }); return; }
    setBusy(true); setNote(null);
    try {
      const r = await fetch("api/wall", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ board, text: body, nick: nick.trim(), passphrase: pass, website: "" }) });
      const d = await r.json().catch(() => ({}));
      if (!r.ok || !d.ok) throw new Error(d.detail || d.error || "HTTP " + r.status);
      setWall((w) => ({ ...w, boards: { ...w.boards, [board]: [d.item, ...(w.boards[board] || [])] } }));
      setText("");
      setNote({ kind: "ok", text: "On the wall - thank you." });
    } catch (err) {
  ...
  return (
    <div className="wall" id="wall">
      ...
      <div className="filter-chips wall-tabs" role="tablist">
        {BOARDS.map((b) =>
          <button key={b.id} type="button" role="tab" className={"chip" + (board === b.id ? " active" : "")} aria-selected={board === b.id} onClick={() => { setBoard(b.id); setShowAll(false); setNote(null); }}>
            {b.label}<span className="n">{(wall.boards[b.id] || []).length}</span>
          </button>
        )}
      </div>
...
function News() {
  ...
  return (
    <section id="posts" className="section">
        <div className="container">
          <div className="section-header reveal">
            <div className="num"><span>02 / 03</span> &nbsp; Posts</div>
        ...
        <div className="container">
          <div>
            <div className="section-header" style={{ gridTemplateColumns: "1fr", gap: 18, marginBottom: 24 }}>
              <div className="num"><span style={{ color: "rgba(255,255,255,0.7)" }}>03 / 03</span> &nbsp; <span style={{ color: "rgba(255,255,255,0.5)" }}>Membership</span></div>
              <h2>Join the <em>Study Group</em>. Open to all UEBS doctoral researchers and beyond.</h2>
            </div>
            <p className="lead">
              This website is managed by the UEBS Statistics Group. Please feel free to email one of the <a href="people.html" style={{ color: "inherit" }}>maintainers</a> or fill in the form to join us. We will add you to the discussion session invites.
            </p>
          </div>
          <div>
            {submitted ?
...
function Footer({ page = "home" }) {
  const home = (hash) => page === "home" ? hash : "index.html" + hash;
  return (
    <footer className="footer">
        <div className="container footer-inner">
          ...
          <div className="footer-links">
            ...
            <a href={home("#talks")}>Research Atlas</a>
            <a href={home("#posts")}>Posts</a>
            <a href="people.html">Maintainers &amp; Contributors</a>
            <a href={home("#join")}>Contact</a>
          </div>
        </div>
      </footer>);
...
function App() {
  ...
  // Maintainers & Contributors moved to a page of its own; links written when
  // it was a section on this page still land in the right place.
  useEffect(() => {
    if (["#maintainers", "#contributors"].includes(window.location.hash)) {
...
// -------- PEOPLE PAGE --------
// Maintainers & Contributors has a page to itself: the field wants the room,
// and the home page keeps to the work - the atlas, then the posts.
function PeoplePage() {
  const [aboutOpen, setAboutOpen] = useState(false);

  useEffect(() => {
    document.documentElement.setAttribute("data-theme", window.TWEAK_DEFAULTS.theme);
  }, []);
  ...
  return (
    <div id="top" data-screen-label="Maintainers and Contributors">
        <a className="skip-link" href="#main-content">Skip to main content</a>
        <Nav page="people" />
        <main id="main-content" tabIndex="-1">
          <Committee />
...
// One bundle serves both pages; the root element says which one this is.
const rootElement = document.getElementById("root");
ReactDOM.createRoot(rootElement).render(
  rootElement.dataset.page === "people" ? <PeoplePage /> : <App />);
